...
import { defaultGraph, GRAPH_SCHEMA, MAX_GRAPH_VERSION, normalizeGraph, VAULT_FORMAT } from "../graph-core.js";
import { MAX_FEEDBACK_NOTE_CHARS, MAX_ZIP_FILES, applyObsidianFeedback, parseObsidianFeedback, parseObsidianVault, readStoredZip } from "../projection-adapter.js";
...
assert.equal(parseObsidianFeedback(`---\ntype: source\nid: doc-source\nlast_reviewed: ${"2".repeat(129)}\n---\n\n# Source`).lastReviewedAt, null, "oversized Obsidian timestamps should be rejected before parsing");
...
assert.equal(vault.files.length, 4);
